...
class Resizer
{
    ...
    public function resize($newWidth, $newHeight, $options = [])
    {
        ...
        if ($this->getOption('matte') != '')
        {
            //Matte transparent parts of image onto specified colors
            $mattecolor = imagecolorallocate($imageResized, hexdec(substr($this->getOption('matte'),0,2)), hexdec(substr($this->getOption('matte'),2,2)), hexdec(substr($this->getOption('matte'),4,2)));
            imagefilledrectangle($imageResized, 0, 0, $optimalWidth, $optimalHeight, $mattecolor);
        }
        else
        {
    ...
    protected function getSizeByAuto($newWidth, $newHeight)
    {
        ...
        if ($this->height < $this->width) {
            $optimalWidth = min($newWidth,$this->width);
            $optimalHeight = $this->getSizeByFixedWidth($optimalWidth);
        }
        // Image to be resized is taller (portrait)
        elseif ($this->height > $this->width) {
            $optimalHeight = min($newHeight, $this->height);
