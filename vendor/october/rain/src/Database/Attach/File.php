...
class File extends Model
{
    ...
    {
        return 'thumb_' . $this->id . '_' . $width . 'x' . $height . '_' . $options['offset'][0] . '_' . $options['offset'][1] . '_' . $options['mode'] . '_' . $options['matte'] . '.' . $options['extension'];
    }
    ...
    {
        if (list($width,$height) = @getimagesize($this->getLocalRootPath().'/'.$this->getDiskPath())) {
            return ($width/$height);
        }
        else{
            return false;
